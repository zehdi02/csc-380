...
static void msg_typed(char *line)
{
	...
	// dhGen(sk_client, pk_client);
	//  send pk_client
	//  Convert mpz_t variable pk to a string

	ssize_t pk_size = mpz_sizeinbase(pk_client, 10); // need to add more space for MAC?

	// Convert the mpz_t buffer to a string
	char *pk_str = (char *)malloc(pk_size);
	mpz_get_str(pk_str, 10, pk_client);

	// Send the buffer size first
	if (send(sockfd, &pk_size, sizeof(pk_size), 0) != sizeof(pk_size))
	{
		// Handle error
	}

	// Send the buffer itself
	if (send(sockfd, pk_str, pk_size, 0) != pk_size)
	{
		// Handle error
	}
	...
	// Convert the mpz_t buffer to a string
	ssize_t y_pk_size;
	if (recv(sockfd, &y_pk_size, sizeof(y_pk_size), 0) != sizeof(y_pk_size))
	{
		// handle error
	}
	char *y_pk_str = (char *)malloc(y_pk_size);

	// receive buffer
	if (recv(sockfd, y_pk_str, y_pk_size, 0) != y_pk_size)
	{
		// handle error
	}
	if (mpz_set_str(pk_server, y_pk_str, 10) < 0)
	{
		// handle error
	}
	string mymsg;
...
void *recvMsg(void *)
{
	...
	while (1)
	{
		...
		mpz_init(sk_client);
		mpz_init(pk_client);
		// communcate the first part from msg_typed
		ssize_t y_pk_size;
		if (recv(sockfd, &y_pk_size, sizeof(y_pk_size), 0) != sizeof(y_pk_size))
		{
			// Handle error
		...
		ssize_t pk_size = mpz_sizeinbase(pk_client, 10);
